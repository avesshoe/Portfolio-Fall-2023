...
```{r}
 lmfit <- lm(formula = cnt ~ season, data = bikes)
tidy(lmfit) %>% kable
```

##### **(b)** How many total coefficients are there in the model?

There are 7552 coefficients total.
    
    
##### **(c)** How many coefficients are estimated for the `season` variable?
  
There are 2604.133 coefficients estimated.
    
    
##### **(d)** Interpret the coefficients of `season` in the model.
   
The  coeffiecients tells  us the average increase in the response variable (in this case the cnt count of total rental bikes). So the estimates of summer are larger than the other seasons, which makes sense because the weather is better and there are more tourists in summer.
...
```{r}
 lmfit <- lm(cnt ~ temp + atemp + mnth + hum + windspeed, data = bikes)
tidy(lmfit) %>% kable
```

##### **(b)** Interpret the coefficients of `temp`, `mnth` and `hum` in the model.

For temp, atemp, and mnth: as they increase by a unit, their estimates also increases.
For hum and windspeed: as they increase, their estimates decrease.

This essentially means that people do not want to ride bikes in humidity and windy conditions, but will more likly ride bikes in warmer temperatures and later months of the year.
    
    
##### **(c)** Which predictors are associated with increased ridership?  Which predictors are associated with decreased ridership?
  
Increased: temp, atemp, mnth
Decreased: hum and windspeed
    
##### **(d)** Which predictors are statistically significant at the 0.05 level?
   
month, humidity, and windspeed are statistically significant at the 0.05 level though atemp is close.
...
```{r}
bike.var.names <- c("temp", "atemp", "mnth", "hum", "windspeed")
#cor_fun <- function(bike.var.names) cor.test(bike.var.names$pr, )
pairs(bikes[,bike.var.names])
```
...
It seems like all the variables are correlated, but temp, atemp, and mnth are more highly correlated.

##### **(c)** Refit your regression model, but this time **omit** the `temp` variable.  Display the coefficients table for this model.

```{r}
bikes2 <- c("atemp", "mnth", "hum", "windspeed")
round(cor(bikes[,bikes2]), 3)
```
...
The coefficient of atemp in the new model is 1, which is not very different than the estimate in part b. Because it is 1, the standard deviation is equal to the mean, so it is statistically significant because it means the data is perfect/ almost perfect.
...
A linear line is not the best way to represent the graph because the trend of the data is more curved. This is becasue there are more bike rentals during the summer months than there are in the colder months. 
